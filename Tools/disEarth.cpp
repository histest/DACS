...
void CEarthAgent::TrackToPosition(const struct SGeographyElements &HLB,double xP[3])
{
	...
	for(unsigned char i=0;i<3;i++)	xP[i] = Xfixed[i];
	InverseRotateFrame(this->Rei,xP);
...
void CEarthAgent::SetRei(const double Time[6])
{
	clsTimer.stcGMT.Year=Time[0];
	clsTimer.stcGMT.Month=Time[1];
	clsTimer.stcGMT.Day=Time[2];
	clsTimer.stcGMT.Hour=Time[3];
	clsTimer.stcGMT.Minute=Time[4];
	clsTimer.stcGMT.Second=Time[5];
	double	MechanicsTime = J2000(&clsTimer.stcGMT)/36525.0;
	clsEarth.Setup(MechanicsTime);
	for (int i=0;i<3;i++)
	{
		for (int j=0;j<3;j++)
		{
			this->Rei[i][j]=clsEarth.Rei[i][j];
		}
	}
}
