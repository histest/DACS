    ...
    FMainWindow(parent)
{
    initUI();
    initConnect();
	connect(centerWindow->uiElements->componentmanagement->ui.inputButton,SIGNAL(clicked()), centerWindow->uiElements->productmanagement, SLOT(centerWindow->uiElements->productmanagement->initUI()));
}
...
void MainWindow::initData(bool IsAdmin,bool IsAuthorized)
{
	if(IsAdmin==false)
	{
		centerWindow->getNavgationBar()->buttons.at(2)->setEnabled(false);
	}
